...
%DIF DEL shell-alias-analysis-oldtmp-8986.tex   Sat May 15 21:29:21 2021
%DIF ADD shell-alias-analysis.tex               Sat May 15 21:25:17 2021
\documentclass[smallextended,natbib]{svjour3}
%DIF 3a3-6
 %DIF > 
\RequirePackage{microtype} %DIF > 
\DisableLigatures{encoding = T1, family = tt* } %DIF > 
 %DIF > 
%DIF -------
...
\usepackage{url}
%DIF 6c10
%DIF < \usepackage[colorlinks,linkcolor=blue,citecolor=blue,urlcolor=blue]{hyperref}
%DIF -------
...
\providecommand{\DIFaddtex}[1]{{\color{blue} #1}} %DIF PREAMBLE
...
A way for these experts to introduce recognizability and customize their command-line experience is to attach distinct names to potentially convoluted, but frequently used, command and argument structures\DIFaddbegin \DIFadd{, as well as workflows expressed as compositions of commands}\DIFaddend .
...
Many users publicly share these configurations on social coding platforms such as GitHub, contributing to a collective knowledge of command-line customizations, which can provide insight into the tasks that expert users repeatedly perform \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend how well the standard environment supports those tasks\DIFdelbegin \DIFdel{, and ways in which this standard environment }\DIFdelend \DIFaddbegin \DIFadd{. 
}

\subsection{\DIFadd{Contribution}}

\DIFadd{We see our large-scale analysis on command-line user customizations manifested in alias definitions as a unique window of opportunity to study how the standard environment of the command line }\DIFaddend could be productively extended\DIFaddbegin \DIFadd{, modified, and improved. 
...
Particularly, our extensive qualitative and quantitative analysis, in conjunction with our dataset, form the basis for identifying opportunities for improving }\DIFaddend command-line \DIFdelbegin \DIFdel{users.
...
}\DIFdelend \DIFaddbegin \DIFadd{experience in the following directions: by characterizing customization practices, we gain a categorical understanding underlying the needs and wants of command-line users; based on our analysis, we identify opportunities for innovation and formulate them as implications, accompanied with concrete scenarios and examples; %DIF >  that aim to support command-line users, tool developers, and the design of the Shell in general. 
further, our comprehensive dataset enables the foundation of learning approaches, as part of learning-based program synthesis, automated repair, and recommendation systems;
...
%DIF > Consequently, we performed an exploratory analysis on the population of alias definitions on GitHub. 
%DIF > Improving the shell holds much promise for development, ops, and data processing
%DIF > To study these preferences, we performed an exploratory analysis on the population of alias definitions on GitHub. 

\DIFaddend \smallskip
\DIFaddbegin \noindent \DIFadd{We summarize the work in this paper as follows:
}\DIFaddend 

\DIFaddbegin \begin{itemize}

	\DIFaddend \item We identified nine \textbf{Customization Practices}, grouped into three high-level themes:
	...
	\item \DIFdelbegin \DIFdel{We formulate }\DIFdelend A \textbf{Curated Dataset of Command-Line Customizations}, consisting of over 2.2 million shell aliases collected from GitHub.
...
We also describe the effective mining technique we used to distill this knowledge, which allowed us to capture almost the whole population (\per{94.09}) of relevant shell configuration files.
	...
	\item We formulate \textbf{Implications for \DIFaddbegin \DIFadd{Improving }\DIFaddend Command-Line \DIFdelbegin \DIFdel{Usability}\DIFdelend \DIFaddbegin \DIFadd{Experience}\DIFaddend } that go beyond single customization practices to address shortcomings and tie them to existing user experience research.
	...
	Overall, we find that many customizations deal with the tension of \emph{Interactivity vs \DIFdelbegin \DIFdel{Batch-Processing}\DIFdelend \DIFaddbegin \DIFadd{Scripting}\DIFaddend }: commands being used to interactively navigate systems, while at the same time being used within scripts for batch-processing.
\end{itemize}
...
\end{CVerbatim}
and the sampling strategy described above, we started by sampling all files in increments of 100 bytes and stopped when we reached 29 KB\DIFaddbegin \DIFadd{, about ten times the median file size of the estimated population encountered so far}\DIFaddend .
...
After removing \DIFdelbegin \DIFdel{duplicates}\DIFdelend \DIFaddbegin \DIFadd{duplicate files based on their SHA-1 hash value}\DIFaddend , our database contains \num{372816} unique files from \num{205126} repositories.
...
%DIF < The majority of aliases in our dataset (\per{86.2}) originate from common startup scripts, like \texttt{.bashrc} or \texttt{.profile} (see Table~\ref{tab:file-names}).
%DIF <  TODO: explain the renaming 14%

%DIF <  \begin{table}
%DIF <  	\caption{Distribution of common file names}
%DIF <      \label{tab:file-names}
%DIF <      \input{table-file-names.tex}
%DIF <  \end{table}
\DIFaddbegin \subsection{\DIFadd{Provenance}}
\DIFaddend 

\DIFaddbegin \DIFadd{The majority of aliases in our dataset (\per{85.74}) originate from common startup scripts, like \texttt{.bashrc}, \texttt{aliases.zsh} or \texttt{.profile} (see Table~\ref{tab:file-names}).
...
\begin{table}\color{blue}
	\caption{\DIFaddFL{Distribution of common file names}}
	\label{tab:file-names}
	\input{table-file-names.tex}
\end{table}

\begin{table}\color{blue}
	\caption{\DIFaddFL{Most common words in repository descriptions}}
	\label{tab:repo-words}
	\input{table-repo-words.tex}	
\end{table}

\DIFadd{Table~\ref{tab:repo-words} shows the most commonly occurring words in repository descriptions on GitHub (excluding stop words), together with the amount of aliases found in repositories whose descriptions contain at least one of these words.
...
\DIFaddend \subsection{Reproducibility}
...
\section{\DIFdelbegin \DIFdel{Findings}\DIFdelend \DIFaddbegin \DIFadd{Analysis}\DIFaddend }
...
\begin{table}
    \centering
    \caption{Top two commands with top arguments and aliases. %DIF > How to read this table: The most common argument for \cmd{git} is \texttt{status}, occurring in \per{5.85} of all aliases mentioning \cmd{git}. Of those, \per{54.27} are declared with the alias name \texttt{gs}, as in \alias{gs}{git status}, and \per{19.19} with the alias name \texttt{gst}, as in \alias{gst}{git status}. All other alias names occur in less than \per{5} of aliases for \texttt{git status}.
		}
    \label{tab:command-summary}
    \input{table-command-summary.tex}
\end{table}

\DIFaddbegin \subsection{\DIFadd{Inductive Coding}}\DIFaddend 

To capture the range of patterns and use cases for which aliases are defined, we \DIFdelbegin \DIFdel{applied inductive codingmethods on a selected cross-section of the dataset}\DIFdelend \DIFaddbegin \DIFadd{analyzed the dataset using inductive coding, a classic technique for qualitative data analysis \citep{saldana:16,thomas:06,dey:03}}\DIFaddend .
Inductive coding is used when conducting exploratory research without prior expectations on themes in the data\DIFdelbegin \DIFdel{\citep{thomas:06}.
...
}\DIFaddend It is an iterative process between theoretical sampling and comparing data within emerging themes\DIFdelbegin \DIFdel{\citep{dey:03}.
We looked at \num{1381} alias definitions derived in a similar way as Table~\ref{tab:command-summary}, i.
e. the }\DIFdelend \DIFaddbegin \DIFadd{, continuing in cycles until no new themes emerge.
}

\DIFadd{Since manually coding the entire dataset is infeasible, we developed our themes by coding a representative sample.
For this sample, we gathered the top three }\DIFaddend most common aliases for the \DIFaddbegin \DIFadd{top ten }\DIFaddend most common arguments for the \DIFdelbegin \DIFdel{most common commands .
In addition, we took }\DIFdelend \DIFaddbegin \DIFadd{top 50 commands (cf.\ Table~\ref{tab:command-summary}), resulting in \num{1381} alias definitions, directly covering \per{28.77} percent of the dataset.
Additionally, we drew }\DIFaddend a random sample of \DIFdelbegin \DIFdel{200 alias definitions }\DIFdelend \DIFaddbegin \DIFadd{\num{200} alias definitions from the long tail of unique aliases.
These are aliases }\DIFaddend that each occur only once in the \DIFdelbegin \DIFdel{dataset to represent the long tail .
Coding was then performed independently by two authors, who labelled each alias definition in the cross-section with descriptive tags, taking }\DIFdelend \DIFaddbegin \DIFadd{entire dataset, making up \per{27.53} of all aliases.
...
\DIFadd{In total, we looked at \num{1581} aliases during the coding process.
In order to reason about the intent of any particular alias, we had to take }\DIFaddend the semantics of \DIFdelbegin \DIFdel{the commands into accountas much as possible}\DIFdelend \DIFaddbegin \DIFadd{each command into account, consulting their \texttt{man} pages and other forms of documentation}\DIFaddend .\footnote{The website \url{https://explainshell.com} has been an indispensable resource.}
\DIFaddbegin \DIFadd{To increase the trustworthiness of our codes, coding was performed independently in parallel by the two authors.
}\DIFaddend After a first iteration, \DIFdelbegin \DIFdel{the coders compared their }\DIFdelend \DIFaddbegin \DIFadd{we compared our }\DIFaddend labels, consolidating different naming conventions.
In consecutive iterations, \DIFdelbegin \DIFdel{the coders }\DIFdelend \DIFaddbegin \DIFadd{we }\DIFaddend identified ways of formalizing the emerged categories, i.e. \DIFdelbegin \DIFdel{, constructing }\DIFdelend \DIFaddbegin \DIFadd{constructing automated }\DIFaddend mechanisms for classifying alias definitions as belonging to certain categories.
The \DIFdelbegin \DIFdel{discussion of the }\DIFdelend \DIFaddbegin \DIFadd{suitability for mechanical classification was an important factor for the viability of any emerging themes.
The discussion of these }\DIFaddend formalizations additionally served to establish a better shared understanding.
Ultimately, \DIFdelbegin \DIFdel{the coders }\DIFdelend \DIFaddbegin \DIFadd{we }\DIFaddend reached a saturation point at which further coding and analysis did not lead to further insights.

\DIFaddbegin \section{\DIFadd{Customization Practices}}
...
\DIFaddbegin \DIFadd{We developed automated classification methods for each practice, which can be found in our replication package.
}\DIFaddend Table~\ref{tab:practices} gives a quantitative overview of the prevalence of each of these practices in the dataset.
...
\input{table-practices-by-command-diff91c13fd18a8.tex}
...
\color{blue}lsd $\rightarrow$ ls -l | grep "\^{}d"
\color{blue}lsd $\rightarrow$ ls -la | grep \^{}d
\color{blue}lsd $\rightarrow$ ls -lGFA --color | grep -i "\^{}d.*/"
\color{blue}lsd $\rightarrow$ ls -lh | grep --color=never '\^{}d'
\end{Verbatim}
%DIF >  or
%DIF >  \begin{Verbatim}[commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7}]
%DIF >  pg $\rightarrow$ ps aux | grep
%DIF >  pg $\rightarrow$ ps -Af | grep \$1
%DIF >  pg $\rightarrow$ ps -ef | grep
%DIF >  pg $\rightarrow$ ps aux | grep -i
%DIF >  \end{Verbatim}
%DIF >  and so on.
\DIFadd{This highlights the highly personal nature of aliases, each customized for an individual use case.
}

\DIFaddend \paragraph{Chaining Subcommands.}
...
\DIFdelbegin \paragraph{\DIFdel{Learning Repair Rules.}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Learning Repair Rules}}
...
This is in line with \DIFaddbegin \DIFadd{visions of integrating collective intelligence in software development \citep{bruch:10}, in particular }\DIFaddend work in leveraging emergent behavior from corpora~\citep{fast:14} that we can codify based on our customization data.
%DIF < An obvious choice from our analysis are autocorrecting aliases to learn repair rules.
\DIFaddbegin \DIFadd{We can also see approaches similar to work on learning code completions from examples \citep{bruch:09}, with our dataset of alias definitions serving as an oracle for an automatic software repair system \citep{monperrus:18} in the domain of shell commands.
}\DIFaddend 

\DIFaddbegin \DIFadd{As an example, take the following erroneous command execution:
}\begin{Verbatim}[breaklines=true,commandchars=\\\{\}]
\color{blue}$ apt-get install vim
...
\end{Verbatim}
\DIFadd{Without having to consult a hard-coded rule involving knowledge about \cmd{apt-get}, or even looking at the specific error that is produced, a command repair system trained on our dataset of alias definitions could easily suggest the correct fix: \texttt{sudo apt-get install vim}.
...
\color{blue}$ systemctl docker status
\color{blue}Unknown command verb docker.
\end{Verbatim}
\DIFadd{The correct invocation is \texttt{systemctl status docker}.
...
%DIF > \footnote{There are \num{652} such aliases in our dataset, out of \num{709} aliases involving \cmd{systemctl} and a \texttt{status} argument.}
indicating the latent knowledge that \texttt{status} is in fact a subcommand of \cmd{systemctl}.
}

\subsection{\DIFadd{Discovering Workflows}}
\label{sec:discovering-workflows}

\DIFaddend Following a different thread of leveraging emergent practices, we can also see how our dataset would enable a world beyond only trying to fix immediate errors, by providing usage hints that could introduce users to common parameters and workflows.
...
For instance, if executing \verb|brew upgrade| results in a failure, we can suggest using \verb|brew update && brew upgrade| instead, based on the patterns in our dataset \DIFaddbegin \DIFadd{(cf. Section~\ref{sec:repair-rules}).
}

\DIFadd{Our findings can also contribute to recent work on the parallelization and distribution of shell scripts.
...
Evidence that this indeed the common case can be found in our dataset, where \per{41.29} of all occurrences of \cmd{comm} follow \texttt{sort | uniq} or \texttt{sort~-u}, and the remainder mostly have unique data sources as input, like \texttt{pacman -Qeq}}\DIFaddend .

\DIFaddbegin \subsection{\DIFadd{Uncovering Conceptual Design Flaws}}
\DIFaddend 
...
\DIFdelbegin \DIFdel{Briefly comparing our findings to the qualitative analysis on conceptual design flaws in \cmd{git} by \cite{perez:13}, we can clearly see the same frustrations with committing and }\DIFdelend \DIFaddbegin \DIFadd{In their analysis of Git, \cite{perez:13,perez:16} describe a number of flaws and operational misfits arising from the conceptual design of the software.
...
\DIFadd{For example, the difficulties some Git users have with the concept of \emph{staging} can be seen in aliases that ensure untracked files are included in a commit by explicitly adding them beforehand, like \alias{commit}{git add . \&\& git commit -m} or \alias{gac}{git add --all \&\& git commit}.}\footnote{\DIFadd{See ``Just Let Me Commit!'' in \cite{perez:13} and ``Incoherent Commit'' in \cite{perez:16}. Confusingly, \texttt{git commit -a}, while performing an implicit \texttt{add}, does not include untracked files.}} \DIFadd{Another frustration is having to use \texttt{git stash} to temporarily save uncommitted changes and clean the working directory in order to avoid conflicts when using other Git commands.
Stashing in itself has no higher purpose in version control, it merely exists as a concept to work around limitations in Git.}\footnote{\DIFadd{See ``I Just Want To Switch Branches!'' in \cite{perez:13}, and especially ``Unmotivated Stash'' and ``Branch Coupling'' in \cite{perez:16}.}}
\DIFadd{This can be seen in aliases like \alias{gspull}{git stash \&\& git pull \&\& git stash pop}, which defines a new type of pull command that stashes away ongoing work before pulling in remote changes and finally re-applying the stashed work.
The same problem happens when }\DIFaddend switching branches, \DIFaddbegin \DIFadd{hence aliases like \alias{gsc}{git stash \&\& git checkout \$1 \&\& git stash pop}.
}

\DIFadd{\cite{church:14} found that version control systems are generally perceived as being risky to use, and sought explanations for this impression via an analysis of Git using a framework of cognitive dimensions \citep{green:96}.
...
Unless one first manually updates Git's local information about remote branches, the command \texttt{git status} will happily report that the local branch is up-to-date with respect to its remote origin, even if the remote repository is in fact many commits ahead}\DIFaddend .
...
\DIFaddbegin \DIFadd{Alias definitions can provide evidence for analytic theories based on cognitive or conceptual models of software use, because they codify workarounds for common annoyances and other customizations based in every-day use.
...
\DIFaddbegin \subsection{\DIFadd{Contextual Defaults}}
...
\DIFaddbegin \DIFadd{This can be the case if the tool assumes a different execution environment than the one it is ultimately used in, e.g. personal notebook vs cloud deployment (where an alias like \alias{java}{java -ea -server} ensures that Java programs are always run on a server-optimized virtual machine) or interactive terminal vs shell script use (cf. Section~\ref{sec:interactivity-vs-scripting}), or if the tool assumes a certain type of user with different needs than the actual user.
}

\DIFaddend Indeed, the variety of different defaults in the data indicate what we call \emph{contextual} defaults, where context could be a reflection of the level of expertise of a command-line user, or a certain persona (e.g., system administrator, data scientist, or software engineer).
\DIFaddbegin \DIFadd{For example, the top default alias for the \cmd{ffmpeg} command is \alias{ffmpeg}{ffmpeg -hide\_banner}, suppressing verbose default output that can be confusing for newcomers but is helpful for the tool developers when providing support and locating errors.}\footnote{\DIFadd{Coincidentally, we also found a ticket in the project's issue tracker requesting this top most default argument from our dataset to become the default option for the command: \url{https://trac.ffmpeg.org/ticket/7211}}}
\DIFaddend We could imagine providing different sets of defaults to different users, effectively alias starter packs, generated from our data.
...
\DIFaddbegin \subsection{\DIFadd{Interactivity vs Scripting}}
\label{sec:interactivity-vs-scripting}
\DIFaddend 

\DIFdelbegin \DIFdel{Context extends to the environment a command is executed in, particularly whether it is used on }\DIFdelend \DIFaddbegin \DIFadd{The first ``modern'' command line, the Bourne shell from 1977, had two primary goals: to provide }\DIFaddend an interactive command \DIFdelbegin \DIFdel{line or as part of a script for batch-processing.
Our findings highlight the }\DIFdelend \DIFaddbegin \DIFadd{interpreter, and at the same time serve as a scripting system \citep{jones:11}.
There is a natural }\DIFaddend tension between these \DIFdelbegin \DIFdel{different ways of execution and how a human user can have needs different from an automated workflow.
This particularly comes to light in the customization practices of \emph{colorizing output} and \emph{overriding defaults}.
While human-readable output or prompting for destructive actions makes no sense in scripts and is usually seen as cumbersome for expert userseven on the command line, our findings show that aliases like \alias{mv}{mv -i} are nonetheless highly desired by some.
}\DIFdelend \DIFaddbegin \DIFadd{two goals, which becomes evident when users are \emph{overriding defaults} with aliases like \alias{mv}{mv~-i}.
...
\DIFaddbegin \DIFadd{The bias of most command-line tools towards scripting is also evident in their output, which is usually minimal and not tailored for human ease-of-use.
...
\DIFadd{Note that the tension between interactivity and scripting is not the same as the divide between ``casual'' and ``power'' users.
...
Related research in the broader context of our work has been conducted on \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{understanding common practices in the software engineering community based on public online data, on the }\DIFaddend use of command-line interfaces and how to improve \DIFdelbegin \DIFdel{it, and }\DIFdelend \DIFaddbegin \DIFadd{them, and on }\DIFaddend the shell as a programming language for both scripting and interactive use.

\DIFdelbegin \DIFdel{The earliest related }\DIFdelend \DIFaddbegin \DIFadd{\cite{zhong:15} conducted an empirical study on real-world bug fixes in Java projects to distill insights that can guide automatic program repair.
\cite{yang:17} mine Stack Overflow posts and GitHub repositories to find out how programmers use and adapt copy-pasted code snippets in open-source projects.
\cite{baltes:19} investigate to what extent such snippets are copied without proper attribution.
...
\DIFadd{The earliest }\DIFaddend study we found \DIFaddbegin \DIFadd{on the use of command-line interfaces }\DIFaddend was by \cite{greenberg:88a}, \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{who }\DIFaddend collected four months of continuous real-life use of the Unix \verb|csh| shell from 168 users. 
The data was used in a follow up study to analyze the use of interactive systems by examining \DIFaddbegin \DIFadd{the }\DIFaddend frequency of command invocations for different groups of users~\citep{greenberg:88b}.
