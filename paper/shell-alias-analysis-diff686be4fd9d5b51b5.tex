...
%DIF DEL shell-alias-analysis-oldtmp-44907.tex   Sat Jul 31 14:55:56 2021
%DIF ADD shell-alias-analysis.tex                Sat Jul 31 14:34:21 2021
...
further, our comprehensive dataset enables the foundation of learning approaches, as part of learning-based program synthesis \DIFaddbegin \DIFadd{\citep{bruch:09,raychev2014completion}}\DIFaddend , automated repair \DIFaddbegin \DIFadd{\citep{monperrus:18}}\DIFaddend , and recommendation systems\DIFaddbegin \DIFadd{~\citep{mens2014source}}\DIFaddend ;
...
\begin{itemize}
	...
	We view our dataset as a playground for fine-grained discovery that can benefit researchers, tool-builders, and command-line users\DIFaddbegin \DIFadd{;
	for example, researchers can use this knowledge base to discover which commands are frequently used together and how they are combined, while tool-builders can see how their programs are being customized}\DIFaddend .
We also describe the effective mining technique we used to distill this knowledge, which allowed us to capture almost the whole population (\per{94.09}) of relevant shell configuration files.
	...
	We are able to \emph{uncover conceptual design flaws}, where customizations indicate frustrations with underlying command structures\DIFaddbegin \DIFadd{, supporting prior research on potential flaws in the conceptual design of certain commands}\DIFaddend ~\citep{perez:13}.
...
The concept of the operating system shell as an independent process executing outside the kernel originated in Multics \DIFaddbegin \DIFadd{\citep{pouzin:65} }\DIFaddend and was further developed into the original Unix shell \texttt{sh} and its various descendants \DIFdelbegin \DIFdel{.
%DIF <   (\TODO references)
}\DIFdelend \DIFaddbegin \DIFadd{\citep{jones:11,seibold2020shell}.
}\DIFaddend The POSIX family of standards defines a Shell Command Language~\citep{posix_standard, greenberg:17}, whose standard implementation is still the \texttt{sh} utility, but there exist a wide variety of popular POSIX-compliant shells like \texttt{bash} or \texttt{zsh}.
...
The names of these configuration files differ by shell, but common ones are \verb|.bashrc|, \verb|.zshrc|\DIFaddbegin \DIFadd{, }\DIFaddend or \verb|.profile| \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{and their main difference is the order in which they are executed.}\footnote{\DIFadd{\url{https://www.gnu.org/software/bash/manual/html_node/Bash-Startup-Files.html} or \url{https://zsh.sourceforge.io/Doc/Release/Files.html}}}
\DIFaddend Often, aliases are also stored in other files referred to by these startup scripts.
...
We used the GitHub Code Search API\DIFaddbegin \footnote{\DIFadd{\url{https://docs.github.com/en/rest/reference/search}}} \DIFaddend to find files written in Shell language\footnote{GitHub uses the Linguist library to classify code: \url{https://github.com/github/linguist}} that contain the string \verb|alias|.
...
\DIFaddbegin \DIFadd{Using our dataset of known-good command invocations, it should be possible to train a statistical language model for command repair, akin to related work in code synthesis~\citep{raychev2014completion}.
}\DIFaddend 
...
Related research in the broader context of our work has been conducted on understanding common practices in the software engineering community based on public online data, on \DIFaddbegin \DIFadd{software configuration in general, on }\DIFaddend the use of command-line interfaces and how to improve them, and on the shell as a programming language for both scripting and interactive use.

\DIFdelbegin \DIFdel{\cite{zhong:15} conducted an empirical study on }\DIFdelend \DIFaddbegin \DIFadd{Empirical studies similar to ours, looking at community knowledge in software engineering to understand practices and distill insights, have been conducted in related domains:
\cite{zhong:15} study }\DIFaddend real-world bug fixes in Java projects to \DIFdelbegin \DIFdel{distill insights that can }\DIFdelend \DIFaddbegin \DIFadd{help }\DIFaddend guide automatic program repair\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend \cite{yang:17} mine Stack Overflow posts and GitHub repositories to find out how programmers use and adapt copy-pasted code snippets in open-source projects\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, while }\DIFaddend \cite{baltes:19} investigate to what extent such snippets are copied without proper attribution\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend \cite{prana:19} conduct a qualitative study to categorize the content of GitHub README files and build an automated classifier to label README sections, easing information discovery\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend \cite{barnaby:20} present a tool that mines code bases for idiomatic usage examples of API methods.

\DIFaddbegin \DIFadd{In the context of software configuration, \cite{sayagh2020survey} surveyed experts and the literature to identify a number of challenges and recommendations related to configuration practices.
...
\DIFaddend The earliest study we found on the use of command-line interfaces was by \cite{greenberg:88a}, who collected four months of continuous real-life use of the Unix \verb|csh| shell from 168 users. 
...
NL2Bash~\DIFdelbegin \DIFdel{\citep{lin:18} }\DIFdelend \DIFaddbegin \DIFadd{\citep{nl2bash} }\DIFaddend implements a system that translates natural language phrases in English to shell commands.
