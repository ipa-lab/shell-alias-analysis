...
Overall, aliases defining default arguments make up a sizable amount of \num{222718} (\per{9.54}) of all alias definitions. 
Interestingly, a majority of these aliases, \num{115476} (\per{51.85}), deal with enabling color in commands (e.g., \verb|--color=auto|, \verb|--color=always|).
The top 3 aliases (or 6 depending on whether you would want to count basic synonyms separately) make up more than \per{95} of the \per{51.85}: \verb|dir|/\verb|vdir| (\per{19.13}), \verb|ls| (\per{28.85}), and \verb|grep|/\verb|fgrep|/\verb|egrep| (\per{49.3}).
...
To quickly summarize the extent within default arguments: \num{24266} aliases define interactivity arguments (or \per{16.97} of overall default argument aliases without color options).
Default arguments that enable human-readable and verbose outputs were present in \num{16356} aliases (or \per{11.44} of overall default argument aliases without color options).
