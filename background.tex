...
The concept of the operating system shell as an independent process executing outside the kernel originated in Multics and was further developed into the original Unix shell \verb|sh| and its various descendants (\TODO references).
The POSIX family of standards defines a Shell Command Language (\TODO reference), whose standard implementation is still the \verb|sh| utility, but there exist a wide variety of popular POSIX-compliant shells like \verb|bash| or \verb|zsh|.
...
So the alias \verb|ll| (present in many system configurations) is used to specify a default argument to a commonly used command.
...
In recent years, people have started sharing their dotfiles on platforms like GitHub (\TODO reference).
