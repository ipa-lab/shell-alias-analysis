...
We would like to rule these non-personal aliases out, so as to better capture user intent.
...
We tried applying certain heuristics, such as looking at file names or the number of aliases per lines in a file, but the results were highly erratic. Taking random samples, we found these heuristics to be no better than chance.

The only reliable indicator we have found is whether the name or description of a repository contain the words ``dotfile'' or ``dotfiles''.
