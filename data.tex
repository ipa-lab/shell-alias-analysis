...
\num{539923} files from \num{209179} repositories, or \per{63.96} per of the original sample, contained aliases.
...
Most (\per{53.38}) come from common startup scripts, like \texttt{.bashrc}, \texttt{aliases.zsh} or \texttt{.profile}.
...
Overall, \per{49.26} per of aliases originate from files of which there are two or more identical copies in the dataset.
In particular, counting each duplicate only once, \emph{ohmyzsh} plugin files contribute only \per{1.95} per of aliases.
