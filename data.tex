...
The majority of aliases in our dataset (\per{53.38}) come from common startup scripts, like \texttt{.bashrc} or \texttt{.profile}.
Furthermore, \per{48.9} of aliases originate in repositories whose name or description contain the words ``dotfile'' or ``dotfiles''.
This is a clear indication of user intent and we can be reasonably certain that these repositories do indeed contain the personal dotfiles of particular users.

It is less clear where the other half of aliases originate.
They might be from personal configuration files that are just not explictily labelled as such, or they might be derived from build scripts or other coincidental sources.
We sampled from the remaining set of files containing alias definitions while not explicitly mentioning the term ``dotfile(s)'' and found the majority to be personal configuration scripts.
However, it is very hard to formalize any criterion that could reliably classify aliases according to user intent.
We have tried applying some heuristics, based on file names and metrics like the ratio of the number of aliases in a file to the number of total lines, but found this to be no better than chance at determining whether a given alias was ``intentional'' or ``coincidental''.

However, when repeating the analyses described in the remainder of this paper on the subset of aliases explicitly labeled as dotfiles, we found that relative frequencies remained virtually unchanged and the overall picture stayed exactly the same (an analysis of both subpopulations can be found in our appendix, see \Cref{sec:reproducibility}).
Thus we are confident that the overwhelming majority of aliases in the dataset originate from intentional personal configuration scripts.

%Some are undoubtedly part of build scripts or utilities, maybe autogenerated or copied from the default configuration of some system distribution.
%Then again, they might also be personal configuration files that are just not explictily labelled as such.
%It is very hard to formalize any criterion that could reliably classify aliases into, say, ``intentional'' and ``coincidental''.
%We have tried applying some heuristics, based on file names and metrics like the ratio of the number of aliases in a file to the number of total lines, but found this to be no better than chance when given random samples.

%However, when we constrained our analysis to only those aliases originating in repositories whose name or description contain the words ``dotfile'' or ``dotfiles'', which represents \per{48.9} of all aliases in the dataset, the results did not significantly differ from those reported in the remainder of this paper.
%Relative frequencies remained virtually unchanged.



%leading us to beleive that our dataset is not significantly skewed



%One strong indication of the intentionality of a great majority of aliases in the dataset is the fact that they have all been publicly shared in personal repositories on GitHub.
%And indeed, if we limit the analysis described in the remainder of this paper to only those aliases coming from repositories explicitly labelled as containing dotfiles, the relative frequencies of our results remain virtually unchanged.


%Noticeably, there is a large amount of files ending in \texttt{.plugin.zsh}, contributing \per{33.68} of all aliases.
...
%It should be noted that \num{255692} of the files in our final dataset (\per{47.36}) are duplicates, meaning they have an SHA hash that is identical to that of another file in the dataset.
%These duplicates derive from \num{18484} distinct files.
%Overall, \per{49.26} of aliases originate from files of which there are two or more identical copies in the dataset.
%In particular, counting each duplicate only once, \emph{ohmyzsh} plugin files contribute only \per{1.95} of aliases.
...
%\input{file-name-distribution-table}
